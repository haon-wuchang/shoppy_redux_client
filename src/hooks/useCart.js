...
export function useCart() {  //custom Hook(커스텀훅)
    ...
    const getCartList = async() => {
        ...
        const result = await axios.post("http://3.39.192.201:9000/cart/items", {"id":id});
    ...
    const saveToCartList = async(formData) => {
        const result = await axios.post("http://3.39.192.201:9000/cart/add", formData);  
    ...
    const updateCartList = async(cid, type) => {     
        const result = await axios.put("http://3.39.192.201:9000/cart/updateQty", 
                                        {"cid":cid, "type": type});
    ...
    const getCount = async() => {
        const id = localStorage.getItem("user_id");
        const result = await axios.post("http://3.39.192.201:9000/cart/count", {"id":id});
    ...
    const deleteCartItem = async(cid) => {
        const result = await axios.delete("http://3.39.192.201:9000/cart/deleteItem", 
                                            {data : {"cid": cid}});
        result.data.result_rows && getCartList();
    }
    ...
    const clearCart = async() => {
        const id = localStorage.getItem("user_id");
        const result = await axios.delete("http://3.39.192.201:9000/cart/clear", {data: {"id": id}});
        result.data.result_rows && getCartList();
    }
